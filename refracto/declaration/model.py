...
class ValueRef:
    ...
    `source` names where the value comes from (``"bind"`` or ``"input"``).
    `key` names the specific entry within that source. This carries no
    expression, arithmetic, or concatenation — it is purely a pointer to one
    value.
    """
    source: str
    key: str
